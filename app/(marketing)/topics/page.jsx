...
export default function TopicsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/topics?tenantSlug=${tenantSlug}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.success && data.topics) {
          const merged = data.topics.map((t) => ({
            name: t.name,
            storiesCount: t.count,
            ...(TOPIC_METADATA[t.name] || {}),
          }));
          setTopics(merged);
        }
      })
            ...
            >
              <div>
                <div className="relative aspect-video w-full rounded-2xl overflow-hidden mb-5 bg-slate-100 border border-slate-100">
                  <Image
                    src={topic.image || UNSPLASH_IMAGES.webDev}
                    alt={topic.name}
                    fill
